package ru.spbau.mit.fl.ast.visitor;
...
//        }
    ...
    public String visitIf(IfStmtNode ifStmtNode) throws IOException {
        ...
        StringBuilder sb = new StringBuilder();
        sb.append("\"#").append(curNodeNumber).append(" ").append("\"");
        String res = sb.toString();
        sb.append(" -> ");
        int temp = sb.length();
        sb.append(stmt2).append(" [label=\"else\"]").append(";\n");
        tree.add(sb.toString());

        sb.replace(temp, sb.length(), exp);
        sb.append(" [label=\"cond\"]").append(";\n");
        tree.add(sb.toString());

        sb.replace(temp, sb.length(), stmt1);
        sb.append(" [label=\"then\"]").append(";\n");
        tree.add(sb.toString());
        return res;
    }
    ...
    public String visitWhile(WhileStmtNode whileStmtNode) throws IOException {
        ...
        sb.append(" -> ");
        int temp = sb.length();
        sb.append(stmt).append(" [label=\"body\"]");
        sb.append(";\n");
        tree.add(sb.toString());
